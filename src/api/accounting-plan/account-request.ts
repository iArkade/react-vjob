...
const createAccountingPlanRequest = async (data: AccountingPlanRequestType) => {
  try {
    const response = await http.post("accounting-plan", {
      code: data.code,
      name: data.name,
...
const getAccountingPlanPaginatedRequest = async (
  page: number,
  limit: number,
  empresa_id: number
): Promise<{ data: AccountingPlanResponseType[]; total: number }> => {
  try {
    const response = await http.get(
      `accounting-plan/paginated?page=${page}&limit=${limit}&empresa_id=${empresa_id}`
    );
    return response.data;
  } catch (error) {
...
const getAccountingPlanRequest = async (
  empresa_id: number
): Promise<AccountingPlanResponseType[]> => {
  try {
    const response = await http.get(
      `accounting-plan/all?empresa_id=${empresa_id}`
    );
    return response.data;
  } catch (error) {
    return handleError(error);
...
const updateAccountingPlanRequest = async (
  id: number,
  data: AccountingPlanRequestType,
  empresa_id: number
) => {
  try {
    const response = await http.put(`accounting-plan/${id}`, {
      code: data.code,
      name: data.name,
      empresa_id,
    });
...
const deleteAccountingPlanRequest = async (
  code: string,
  empresa_id: number
) => {
  try {
